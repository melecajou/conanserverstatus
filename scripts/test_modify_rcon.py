...
async def test_modify_command(server_alias, char_name, slot, new_qty):
    # 1. Achar o servidor
    server_conf = next((s for s in config.SERVERS if s.get("ALIAS") == server_alias or s["NAME"] == server_alias), None)
    if not server_conf:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 5:
        print("Uso: python3 test_modify_rcon.py 'AliasServidor' 'NomeChar' 'Slot' 'NovaQtd'")
        sys.exit(1)
