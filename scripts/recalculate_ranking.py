...
def recalculate():
    ...
    for server_conf in config.SERVERS:
        ...
        if not db_path or not os.path.exists(db_path):
            logging.warning(f"Banco de dados não encontrado para {server_name}: {db_path}")
            continue
        ...
        try:
            ...
            con.close()
            logging.info(f"Finalizado {server_name}: {kills_count} abates PvP processados.")
            ...
            if last_event_file and max_event_time > 0:
                os.makedirs(os.path.dirname(os.path.abspath(last_event_file)), exist_ok=True)
                with open(last_event_file, 'w') as f:
                    f.write(str(max_event_time))
                logging.info(f"Arquivo de sincronização atualizado para {server_name}: {max_event_time}")

        except Exception as e:
            logging.error(f"Erro ao processar {server_name}: {e}")

if __name__ == "__main__":
    print("Este script irá ZERAR o ranking atual e recalcular tudo com base nos game.db de cada servidor.")
