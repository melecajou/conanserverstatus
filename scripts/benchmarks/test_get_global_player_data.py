...
async def test_get_global_player_data():
    ...
    pids = ["pid1", "pid2"]
    data = await get_global_player_data(pids, global_db_path=db_path)
    ...
    data = await get_global_player_data(["pid3"], global_db_path=db_path)
    ...
    data = await get_global_player_data(["pid1", "pid3", "pid4"], global_db_path=db_path)
    assert data["pid1"]["discord_id"] is None
...
if __name__ == "__main__":
    try:
        asyncio.run(test_get_global_player_data())
