...
TEMP_DIR = os.path.join(PROJECT_ROOT, "temp_killfeed_all_bench")
...
def setup_servers(num_servers=5):
    ...
    # Create Ranking DB and tables since ranking update needs it
    con = sqlite3.connect(config.KILLFEED_RANKING_DB)
    cur = con.cursor()
    cur.execute("""
    ...
    for i in range(num_servers):
        ...
        # Add 100 events to each server to simulate some work
        events = []
        for j in range(100):
            world_time = 100 + j
            event_type = 103
    ...
    config.KILLFEED_UNIFIED_RANKINGS = [
        {"title": f"Unified {i}", "enabled": True, "channel_id": 999 + i, "servers_to_include": ["Server_0", "Server_1"]} for i in range(5)
    ]
...
async def main():
    ...
    # Pre-populate some scores so ranking queries don't fail immediately
    con = sqlite3.connect(config.KILLFEED_RANKING_DB)
    cur = con.cursor()
    scores = []
    for s in config.SERVERS:
        for i in range(10):
            scores.append((s["NAME"], f"Player{i}", i, i, i))
    cur.executemany("INSERT INTO scores VALUES (?, ?, ?, ?, ?)", scores)
    con.commit()
    con.close()

    print(f"Benchmarking Sequential tasks over {len(config.SERVERS)} servers...")

    start_time = time.perf_counter()
    await cog.kill_check_task.coro(cog)
    await cog.ranking_update_task.coro(cog)
    await cog.unified_ranking_task.coro(cog)
    end_time = time.perf_counter()
    seq_time = end_time - start_time

    print(f"Sequential Execution Time: {seq_time:.4f}s")

    # Apply optimizations (Gather)
    ...
    async def new_ranking_update_task():
        async def safe_process(server_conf):
            try:
                await cog._update_server_ranking(server_conf)
            except Exception as e:
                pass
    ...
    async def new_unified_ranking_task():
        async def safe_process(u_config):
            try:
                await cog._update_unified_ranking(u_config)
            except Exception as e:
                pass
    ...
    setup_servers(10) # Reset DBs just in case

    print(f"Benchmarking Gathered tasks over {len(config.SERVERS)} servers...")
    start_time = time.perf_counter()
    await new_kill_check_task()
    await new_ranking_update_task()
    await new_unified_ranking_task()
    end_time = time.perf_counter()
    gather_time = end_time - start_time

    print(f"Gather Execution Time: {gather_time:.4f}s")
