...
def get_clan_audit(clan_name_filter):
    ...
    cursor.execute("SELECT guildId, name FROM guilds WHERE name LIKE ?", (f"%{clan_name_filter}%",))
    ...
    for guild_id, real_clan_name in guilds:
        ...
        sorted_items = sorted(inventory_summary.items(), key=lambda x: x[1], reverse=True)
