...
def get_universal_dna(slot):
    ...
    res = conn.execute(
        "SELECT template_id, hex(data) FROM item_inventory WHERE owner_id=? AND item_id=? AND inv_type=0",
        (CHAR_ID, slot),
    ).fetchone()
    conn.close()
    if not res:
...
async def run():
    ...
    try:
        ...
        before = set(
            conn.execute(
                "SELECT item_id, inv_type FROM item_inventory WHERE owner_id=?",
                (CHAR_ID,),
            ).fetchall()
        )
        conn.close()
        ...
        conn = sqlite3.connect(f"file:{DB_PATH}?mode=ro", uri=True)
        after = conn.execute(
            "SELECT item_id, inv_type FROM item_inventory WHERE owner_id=?", (CHAR_ID,)
        ).fetchall()
        conn.close()
        ...
        if new:
            ...
            for p_id, v in dna["int"].items():
                await client.send_cmd(
                    f"con {idx if 'idx' in globals() else 0} SetInventoryItemIntStat {n_slot} {p_id} {v} {i_type}"
                )
            for p_id, v in dna["float"].items():
                await client.send_cmd(
                    f"con {idx if 'idx' in globals() else 0} SetInventoryItemFloatStat {n_slot} {p_id} {v} {i_type}"
                )
