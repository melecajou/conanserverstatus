        ...
        if offset + 4 <= len(data):
            ...
            if 0 < length < 255 and offset + 4 + length <= len(data):
                try:
                    ...
                    if (
                        all(32 <= ord(c) <= 126 for c in potential_string)
                        and len(potential_string) > 3
                    ):
                        print(
                            f"String encontrada no offset {offset}: {potential_string}"
                        )
                        offset += 4 + length
                        continue
                except:
            ...
            if 0 < val2 < 20:
                ...
                for i in range(val2):
                    if offset + 8 <= len(data):
                        ...
                        print(
                            f"  Prop {i+1}: ID={p_id:<3} | {val_str:<15} | Hex={p_hex.hex()}"
                        )
                        offset += 8
                continue
