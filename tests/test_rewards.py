...
class TestRewardsCog(IsolatedAsyncioTestCase):
    ...
    async def test_regular_player_receives_reward(self):
        ...
        self.rewards_cog._fetch_batch_player_data = AsyncMock(
            return_value={
                "steam_1": {
                    "online_minutes": 65,
                    "last_reward_playtime": 0,
    ...
    async def test_fetch_batch_player_data_merges_correctly(self):
        ...
        with patch(
            ...
            self.assertEqual(
                batch_data["steam_1"]["online_minutes"], 65
            )  # From setUp PLAYER_DATA
            self.assertEqual(
                batch_data["steam_1"]["discord_id"], "discord_1"
            )  # From mock_global

            # steam_2: Merged correctly
            self.assertIn("steam_2", batch_data)
            self.assertEqual(
                batch_data["steam_2"]["online_minutes"], 35
            )  # From setUp PLAYER_DATA
