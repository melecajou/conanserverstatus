...
class TestRegistrationSync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sync_roles_command_cache_miss(self):
        bot = MagicMock()
        ...
        with patch("config.REGISTERED_ROLE_ID", 999, create=True):
            with patch.object(RegistrationCog, "_fetch_all_linked_discord_ids", return_value={12345}):
                await cog.sync_roles_command.callback(cog, interaction)

                interaction.guild.get_member.assert_called_with(12345)
                interaction.guild.fetch_member.assert_called_with(12345)
                member.add_roles.assert_called_with(role)

    async def test_sync_roles_command_cache_hit(self):
        bot = MagicMock()
        with patch("discord.ext.tasks.Loop.start"):
            cog = RegistrationCog(bot)
        ...
        with patch("config.REGISTERED_ROLE_ID", 999, create=True):
            with patch.object(RegistrationCog, "_fetch_all_linked_discord_ids", return_value={12345}):
                ...
                interaction.guild.get_member.assert_called_with(12345)
                interaction.guild.fetch_member.assert_not_called()
                member.add_roles.assert_called_with(role)
