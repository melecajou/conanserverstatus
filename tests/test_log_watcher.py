...
class TestLogWatcher(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
    ...
    async def test_init_starts_at_end(self):
        watcher = LogWatcher(self.log_path)
        # First call initializes and consumes nothing
        new_lines = await watcher.read_new_lines()
    ...
    async def test_read_multiple_lines(self):
        watcher = LogWatcher(self.log_path)
        await watcher.read_new_lines()  # Init
    ...
    async def test_missing_file(self):
        ...
        self.assertEqual(await watcher.read_new_lines(), [])
        ...
        self.assertEqual(await watcher.read_new_lines(), ["Appended"])
