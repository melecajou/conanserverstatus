        ...
        for server_conf in config.SERVERS:
            ...
            try:
                ...
                for schedule_item in ann_config.get("SCHEDULE", []):
                    if (
                        schedule_item.get("DAY") == current_day_name
                        and schedule_item.get("HOUR") == now.hour
                    ):
                        announcement_key = (server_conf["NAME"], schedule_item["DAY"])
                        if self.announcements_sent_today.get(announcement_key) != today:
                            channel = self.bot.get_channel(channel_id)
                            if channel:
                                message = schedule_item.get(
                                    "MESSAGE", self.bot._("Scheduled announcement.")
                                )
                                await channel.send(message)
                                logging.info(
                                    self.bot._(
                                        "Sent announcement for '{server_name}' to channel {channel_id}."
                                    ).format(
                                ...
                                logging.error(
                                    self.bot._(
                                        "Could not find announcement channel with ID {channel_id} for server '{server_name}'."
                                    ).format(
                                        channel_id=channel_id,
                                        server_name=server_conf["NAME"],
                                    )
                ...
                logging.error(
                    self.bot._(
                        "Error in announcement task for server '{server_name}': {error}"
